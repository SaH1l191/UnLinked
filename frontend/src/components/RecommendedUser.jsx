...
const RecommendedUser = ({ user }) => {
  ...
  const { mutate: rejectRequest } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["connectionStatus", user._id] });
      //mark as stale state so to refetch again
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || "An error occurred");
    },
  ...
  const renderButton = () => {
		if (isLoading) {
			return (
				<button className='px-3 py-1 text-sm text-gray-500 bg-gray-200 rounded-full' disabled>
					Loading...
				</button>
			);
		}
  ...
  return (
    <div className='flex items-center justify-between mb-4'>
      ...
      {/* as it includes many case handling logic in a button  */}
      {renderButton()}
    </div>
  )
}
